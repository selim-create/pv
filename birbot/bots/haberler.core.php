...
class Birbot extends BirbotHelper
{
        ...
        <div class="bib-list">
            <form id="bibCronForm" action="javascript:;" method="post">
                <label>Kaynak Kategori</label><select name="category" class="bib-input cronCat">
                    <option value="https://www.haberler.com/anne-cocuk/">Anne Çocuk</option>
                    <option value="https://www.haberler.com/dunya/">Dünya</option>
                    <option value="https://www.haberler.com/egitim/">Eğitim</option>
                    <option value="https://www.haberler.com/ekonomi/">Ekonomi</option>
                    <option value="https://www.haberler.com/guncel/">Güncel</option>
                    <option value="https://www.haberler.com/guzellik/">Güzellik</option>
                    <option value="https://www.haberler.com/kultur-sanat/">Kültür Sanat</option>
                    <option value="https://www.haberler.com/lezzet/">Güncel</option>
                    <option value="https://www.haberler.com/moda/">Moda</option>
                    <option value="https://www.haberler.com/otomobil/">Otomobil</option>
                    <option value="https://www.haberler.com/politika/">Politika</option>
                    <option value="https://www.haberler.com/saglik/">Sağlık</option>
                    <option value="https://www.haberler.com/spor/">Spor</option>
                    <option value="https://www.haberler.com/tatil/">Tatil</option>
                    <option value="https://www.haberler.com/teknoloji/">Teknoloji</option>
                </select><br />
                <input type="hidden" class="botCore" value="haberler" />
    ...
    function list()
    {
        ...
        <div class="bib-list">
            <form id="bibForm" action="javascript:;" method="post">
                ...
                <label>Sayfa</label><input type="text" name="page" class="bib-input" placeholder="Sayfa Numarası" value="1" /><br />
                <input type="hidden" name="action" value="birbot_data" />
                <input type="hidden" name="core" value="haberler" />
    ...
    function data()
    {
        ...
        $page     = $_POST['page'];
        if($category == "https://www.haberler.com/spor/"){
            $kaynak = $this->get_url_curl($category);
            preg_match_all('@class="hbBoxMainText">                <a href="(.*?)" title="@si', $kaynak, $kLink);
        }else{
            $kaynak = $this->get_url_curl($category."s".$page."/");
            preg_match_all('@<a data-lazyappeared="(.*?)" href="(.*?)" title="(.*?)" ><div class="hbBoxImage brdFill">@si', $kaynak, $kLink);
        }
        ...
        <div class="bib-dataArea">
            ...
            <?php foreach($kLink[2] as $key_link=>$value_link):
                ...
                preg_match_all('@<div class="hbptContent haber_metni">(.*?)</div>@si', $kn, $content);
    ...
    function cron()
    {
        ...
        foreach ($categories as $key => $value) {
            ...
            if($kategori == "https://www.haberler.com/spor/"){
                $kaynak = $this->get_url_curl($kategori);
                preg_match_all('@class="hbBoxMainText">                <a href="(.*?)" title="@si', $kaynak, $kLink);
            }else{
                $kaynak = $this->get_url_curl($kategori);
                preg_match_all('@class="hbBoxMainText"><a href="(.*?)" title="@si', $kaynak, $kLink);
