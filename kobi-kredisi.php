...
if (!empty($_GET['type'])) {
    ?>
<script>
    window.location.href = '<?=home_url() . "/" . $bp_options['page_kobikredisi'] . "/" . permalink_bf(str_replace(array("{ay}", "{tutar}"), array($_GET['vade'], str_replace(".", "", $_GET['tutar'])), $bp_options['krediSorgulamaRewrite']))?>'
</script>
<?php
}
...
$new_title = str_replace(array('{kredi}', '{vade}', '{tutar}'), array("Kobi Kredisi", $parameters['vade'], $parameters['tutar']), $bp_options['page_kredi_seo']) . " - " . get_bloginfo('name');;
