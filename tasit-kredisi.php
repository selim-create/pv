...
*/
        ...
        if (!empty($_GET['type'])) {
            ?>
<script>
            window.location.href = '<?=home_url() . "/" . $bp_options['page_tasitkredisi'] . "/" . permalink_bf(str_replace(array("{ay}", "{tutar}"), array($_GET['vade'], str_replace(".", "", $_GET['tutar'])), $bp_options['krediSorgulamaRewrite']))?>';
</script>
<?php
        }
        ...
        $new_title = str_replace(array('{kredi}', '{vade}', '{tutar}'), array("Taşıt Kredisi", $parameters['vade'], $parameters['tutar']), $bp_options['page_kredi_seo']) . " - " . get_bloginfo('name');;
                    ...
                    ?>
                        ...
                        <div class="nkcont-li <?= $special_class ?>">
                            <div class="nkcont-li-left">
                                <figure>
                                    <?php if ($banka_img == "alj-finans") { ?>
                                        <img src="<?php bloginfo("template_directory"); ?>/img/banka/<?= $banka_img ?>.png" alt="a">
                                    <?php } else { ?>
                                        <img src="<?php bloginfo("template_directory"); ?>/img/banka/<?= $banka_img ?>.svg" alt="a">
                                    <?php } ?>
                                </figure>
                            ...
                            <ul>
                                <li>
                                    <b>Faiz Oranı</b>
                                    <span>%<?= $json_data['faiz'][$key] ?> <i class="question question22">
                                            <div class="info">
                                ...
                                <li>
                                    <b>Toplam Ödeme</b>
                                    <span><?= $json_data['toplam_odeme'][$key] ?><i class="question">
                                            <div class="info">
