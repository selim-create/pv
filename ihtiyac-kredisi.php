...
if (!empty($_GET['type'])) {
    ?>
<script>
    window.location.href = '<?=home_url() . "/" . $bp_options['page_ihtiyackredisi'] . "/" . permalink_bf(str_replace(array("{ay}", "{tutar}"), array($_GET['vade'], str_replace(".", "", $_GET['tutar'])), $bp_options['krediSorgulamaRewrite']))?>';
</script>
<?php
}
...
$new_title = str_replace(array('{ay}', '{kredi}', '{vade}', '{tutar}'), array($_GET['vade'], "İhtiyaç Kredisi", $parameters['vade'], $parameters['tutar']), $bp_options['krediSorgulamaRewrite']) . " - " . get_bloginfo('name');
            ...
            <div class="nkcont-in">
                <?php if (empty($json_data['banka'][0])) {
                    ...
                    foreach ($list_type as $key => $data):
                        ...
                        $json_data['aylik_taksit'][$key] = str_replace(array(".", ","), array("", "."), $json_data['aylik_taksit'][$key]);
                        if ($i == 0) {
                            $special_class = "ozel";
                        } else {
                            $special_class = "";
                    ...
                    <?php } ?>
                        ...
                        <div class="nkcont-li <?= $special_class ?>">
                            <div class="nkcont-li-left">
                                ...
                                <span class="nkcont-svg"><b><u></u><em><?= $json_data['kredi'][$key] ?></em></b></span>
                            </div>
                            <ul>
                                <li>
                                    <b>Faiz Oranı</b>
        ...
        <div class="info">
            ...
            <ul>
                <li>Faiz oranı, kredi notunuza göre değişebilir.</li>
                                ...
                                <li>
                                    <b>Aylık Taksit</b>
                                    <span><?= number_format($json_data['aylik_taksit'][$key], 2, ",", ".") ?> TL</span>
                                </li>
                                <li>
                                    <b>Toplam Ödeme</b>
